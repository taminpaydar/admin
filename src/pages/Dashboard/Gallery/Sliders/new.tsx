...
export default function newArticleGroup() {
    ...
    const onSubmit: SubmitHandler<Inputs> = async ss => {
        try {
            let res: any = await axios({
                method: 'post',
                url: config.url + '/v1/dashboard/slider',
                data: data,
                headers: {
            ...
            let mydata = res.data;
            router.push('/Dashboard/Gallery/Sliders/' + res.data.data.id)
            return mydata;
        } catch (error: any) {
            let x = key + 1;
            setKey(x);
    ...
    return <Layout>
        <Container>
            ...
            <Typography fontSize={'28px'} mt={4} mb={3} fontWeight={'bold'}>{i18n.t('New Slider')}</Typography>
